...
function getTeamAbbrevMap(pbp) {
  ...
  if (awayId && awayAbbrev)
    map.set(Number(awayId), String(awayAbbrev).toUpperCase());
  if (homeId && homeAbbrev)
    map.set(Number(homeId), String(homeAbbrev).toUpperCase());
  return map;
}
...
function getGoalPlays(pbp) {
  return (Array.isArray(pbp?.plays) ? pbp.plays : []).filter(
...
function upsertGoalRecord(store, teamAbbrev, playerId, goalRecord) {
  ...
  const existingIdx = player.goals.findIndex(
    (g) =>
      Number(g.gameId) === Number(goalRecord.gameId) &&
      Number(g.goalIndex) === Number(goalRecord.goalIndex),
  );
  if (existingIdx >= 0) {
...
async function run() {
  ...
  const archiveEntries = await fs.readdir(args.inputDir, {
    withFileTypes: true,
  });
  ...
  for (let gi = 0; gi < gameDirs.length; gi++) {
    ...
    try {
      ...
      const homeTricode =
        String(
          pbp?.homeTeam?.abbrev || pbp?.homeTeam?.triCode || "",
        ).toUpperCase() || null;
      const awayTricode =
        String(
          pbp?.awayTeam?.abbrev || pbp?.awayTeam?.triCode || "",
        ).toUpperCase() || null;
      const gameDate =
        pbp?.gameDate || String(pbp?.startTimeUTC || "").slice(0, 10) || null;
      ...
      for (let i = 0; i < goals.length; i++) {
        ...
        const teamAbbrev = getTeamAbbrevForGoal(
          play,
          details,
          teamById,
          playerTeamById,
        ...
        if (await fileExists(goalFilePath)) {
          try {
            ...
            if (Array.isArray(replayFrames) && replayFrames.length > 0) {
              ...
              goalFrameIdx =
                out?.meta?.goalFrameIdx ?? out?.meta?.goamFrameIx ?? null;
        ...
        const goalRecord = {
          ...
          date: gameDate,
          opponentTricode:
            teamAbbrev === homeTricode
              ? awayTricode
              : teamAbbrev === awayTricode
                ? homeTricode
                : null,
          shotType: details?.shotType ?? null,
          clip: {
            en: details?.discreteClip,
            fr: details?.discreteClipFr,
            enFull: details?.highlightClip,
            frFull: details?.highlightClipFr,
          },
          goalClassification,
        };
        upsertGoalRecord(teamStore, teamAbbrev, scorerId, goalRecord);
        const gamedayPath = getGamedayGoalPath(
          homeTricode,
          awayTricode,
          gameId,
          goalIndex,
  ...
  console.log(
    `Goals skipped (missing scorer/replay/non-NHL team/shootout): ${skippedGoals}`,
  );
  console.log(
    `Goals with unknown classification (classification.primary=null): ${unknownClassificationGoals.length}`,
  );
  console.log(
